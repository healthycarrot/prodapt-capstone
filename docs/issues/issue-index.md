...
| [#28](https://github.com/healthycarrot/prodapt-capstone/issues/28) | CLOSED | FR-06 rollback: `/search` の temporary raw Mongo response 撤去。 |
| [#29](https://github.com/healthycarrot/prodapt-capstone/issues/29) | OPEN | Presentation topic memo。プレゼンのストーリーと各スライドの論点整理。 |
| [#30](https://github.com/healthycarrot/prodapt-capstone/issues/30) | CLOSED | FR-06/FR-09 実装Issue: ESCO suggest API と frontend hard filter 候補制約（`/search` 契約維持）。 |
...
## Update Log (2026-03-17): Issue #29 Presentation Memo Sync
- Source: Issue #29 body
  - https://github.com/healthycarrot/prodapt-capstone/issues/29
- Presentation direction:
  - Storyline should flow from problem framing -> design thinking -> architecture -> trade-off decisions -> preprocessing pipeline -> retrieval -> agent scoring -> ROI.
- Slide-specific guidance relevant to current docs update:
  - Enterprise architecture should emphasize modularity, `/search` as current frontend entrypoint, and `/retrieve` as a separable retrieval boundary.
  - Trade-off slide should explain the long-form resume bottleneck and justify storage/retrieval choices with explicit database comparisons.
  - System thinking should connect multi-format unstructured data to the decision to invest in preprocessing and normalization.
...
## Update Log (2026-03-18): Issue #22 Frontend React Eval Case Label Fix
- Related issues checked:
  - #22 (FR-08 quality evaluation)
  - #20 (FR-06 API contract)
  - #21 (FR-07 input validation)
- Problem:
  - `frontend_react_typescript` returned `422` because the explicit filters used labels rejected by the current ESCO lexical validation:
    - `skill_terms=['React']`
    - `occupation_terms=['frontend developer']`
- Fix:
  - updated the eval fixture and request-body sample to use currently valid explicit labels:
    - `skill_terms=['JavaScript', 'TypeScript', 'CSS']`
    - `occupation_terms=['web developer']`
  - kept `query_text` mentioning `React` so the search intent still reflects the original frontend stack
- Rationale:
  - explicit filter fields must satisfy the API's exact/alt ESCO label contract and otherwise return `422`
  - using valid explicit filters preserves the scenario as a frontend web search while removing artificial validation failure from the eval dataset

## Update Log (2026-03-18): Issue #22 Faithfulness Improvement via Grounded Summary Template
- Related issues checked:
  - #22 (FR-08 quality evaluation)
  - #19 (FR-05 explainable results)
  - #18 (FR-04 agent scoring)
- Problem:
  - `recommendation_summary` was previously built by concatenating top agent `reason` strings
  - this made the summary prone to unsupported soft-skill, seniority, or recency claims and hurt `FaithfulnessMetric`
- Fix:
  - changed `/search` result mapping to compose `recommendation_summary` from structured evidence first:
    - `matched_skills`
    - `transferable_skills`
    - `experience_matches`
    - `major_gaps`
    - candidate `occupation_labels` as a fallback alignment hint
  - freeform agent reason text is now only used as a final fallback when structured evidence is unavailable
- Expected effect:
  - make summary wording more deterministic and closer to evidence already exposed in the response payload
  - reduce unsupported narrative claims from soft-skill and career-progression agent reasons
- Verification:
  - added unit coverage for grounded summary composition and `/search` row mapping
  - existing `/search` API and output-audit tests still pass
  - live Faithfulness re-run was attempted, but the local MongoDB connection was unavailable during that verification step
- Validation:
  - passed:
...
## Update Log (2026-03-18): Issue #22 Rollback of Grounded Summary Composition
- Related issues checked:
  - #22 (FR-08 quality evaluation)
  - #19 (FR-05 explainable results)
- Decision:
  - rolled back the grounded-summary rewrite for `recommendation_summary`
  - restored `/search` result mapping to use the original agent-provided `row.recommendation_summary`
- Code changes:
  - reverted [search_orchestration.py](C:/Users/Administrator/Desktop/prodapt-capstone/backend/app/services/search_orchestration.py) to the pre-`f517013` summary mapping behavior
  - removed the grounded-summary-only unit test file `backend/tests/test_search_summary_composer.py`
- Rationale:
  - the user requested to revert this behavioral change before further work continues
  - keeping the explainability contract stable takes priority over the faithfulness experiment in the current branch
