...
DEFAULT_CSV_PATH = REPO_ROOT / "data" / "4th_data" / "Resume.csv"
...
def summarize(rows: list[ResumeAnalysis]) -> AggregateSummary:
    return AggregateSummary(
        ...
        rows_with_html=sum(1 for row in rows if row.html_length > 0),
        median_text_length=statistics.median(row.text_length for row in rows),
        median_html_length=statistics.median(row.html_length for row in rows),
        ...
        breakdown.append(
            {
                ...
                "pct_with_html": round(
                    100 * sum(1 for item in items if item.html_length > 0) / len(items),
                    2,
                ),
            }
        )
...
def print_report(summary: AggregateSummary, samples: dict[str, list[ResumeAnalysis]], top_categories: list[dict[str, object]]) -> None:
    print("=== 4th Resume.csv Structure Analysis ===")
    ...
    print(f"rows_with_html: {summary.rows_with_html}")
    print(f"median_text_length: {summary.median_text_length}")
    print(f"median_html_length: {summary.median_html_length}")
    ...
    for item in top_categories[:10]:
        print(
            ...
            f"pct_with_sections={item['pct_with_sections']}%, pct_with_dates={item['pct_with_dates']}%, pct_with_html={item['pct_with_html']}%"
        )
    ...
    for sample_name, sample_rows in samples.items():
        ...
        for row in sample_rows:
            print(
                ...
                f"newlines={row.newline_count} | html_length={row.html_length}"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze how much structural signal exists in the 4th Resume.csv Resume_str fields."
    )
    parser.add_argument(
        "--csv-path",
        default=str(DEFAULT_CSV_PATH),
        help="Path to the 4th Resume.csv",
    )
    parser.add_argument(
        "--sample-size",
        type=int,
